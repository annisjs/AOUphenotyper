...
gerd <- function(dataset,output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
  ...
  result_all <- setDT(result_all)[,.(anxiety_entry_date = min(condition_start_date),
                                     anxiety_status = length(condition_start_date) > 0),
                                  .(person_id)]
  fwrite(result_all,file="anxiety.csv")
  system(str_glue("gsutil cp anxiety.csv {output_folder}/anxiety.csv"),intern=TRUE)
}
