...
closest_glucose <- function(dataset,output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
  result_all <- lab_query(dataset,c("Glucose [Mass/volume] in Serum or Plasma",
  ...
  result_all <- result_all[,.(closest_glucose_entry_date = measurement_date[1],
                              closest_glucose_value = value_as_number[1]),.(person_id,anchor_date)]
  fwrite(result_all,file="closest_glucose.csv")
  system(str_glue("gsutil cp closest_glucose.csv {output_folder}/closest_glucose.csv"),intern=TRUE)
}
