...
#' Format clustering data for time varying Cox model.
#'
#' @param fitbit data.table with columns: person_id, daily_class (can range from 1 to 5).
#' @param dx data.table with columns: person_id, dx_entry_date, dx_status
#' @param last_medical_encounter data.table with columns: person_id, last_medical_encounter_entry_date
#'
#' @return A data.table
...
format_clust_cox <- function(clust_dat,dx,last_medical_encounter,verbose=TRUE)
{
  ...
  merged_cox_baseline_agg <- merged_cox_baseline[,.(baseline_steps = mean(steps),
                                                    ...
                                                    baseline_c5 = length(which(daily_class == 5))/length(daily_class),
                                                    baseline_mean_dc = mean(as.numeric(daily_class)),
                                                    count = length(date)),.(person_id)]
  merged_cox_baseline_agg <- merged_cox_baseline_agg[count >= 15]
  ...
  #aggregate by month
  merged_cox_agg <- merged_cox[,.(c1 = length(which(daily_class == 1))/length(daily_class),
                                  c2 = length(which(daily_class == 2))/length(daily_class),
                                  c3 = length(which(daily_class == 3))/length(daily_class),
                                  c4 = length(which(daily_class == 4))/length(daily_class),
                                  c5 = length(which(daily_class == 5))/length(daily_class),
                                  mean_dc = mean(as.numeric(daily_class)),
                                  baseline_c1 = baseline_c1[1],
                                  baseline_c2 = baseline_c2[1],
                                  baseline_c3 = baseline_c3[1],
  ...
  # The code block above assumes there's at least 2 months of data, so we have to remove that artifact.
  merged_cox_agg[,data_points := length(count),.(person_id)]
  merged_cox_agg[,repeated := data_points == 2 & (c1[1] == c1[2]),.(person_id)]
  merged_cox_agg[data_points == 2 & repeated, time2 := time2[length(time2)],.(person_id)]
  merged_cox_agg[data_points == 2 & repeated, time1 := c(time1[1],time2[1]),.(person_id)]
  merged_cox_agg[,seq := 1:length(count),.(person_id)]
