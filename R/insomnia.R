...
#' @export
insomnia <- function(dataset,output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
  ...
  result_all <- setDT(result_all)[,.(insomnia_entry_date = min(condition_start_date),
                                     insomnia_status = length(condition_start_date) > 0),
                                  .(person_id)]
  fwrite(result_all,file="insomnia.csv")
  system(str_glue("gsutil cp insomnia.csv {output_folder}/insomnia.csv"),intern=TRUE)
}
