...
#' @export
closest_non_hdl <- function(dataset,output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
  if (is.null(anchor_date_table))
  {
    stop("closest_non_hdl is not a primary variable and requires an anchor date table.")
  }
  result_all <- lab_query(dataset,c("Cholesterol non HDL [Mass/volume] in Serum or Plasma"))
  ...
  fwrite(result_all,file="closest_non_hdl.csv")
  system(str_glue("gsutil cp closest_non_hdl.csv {output_folder}/closest_non_hdl.csv"),intern=TRUE)
}
