...
#' @return output_folder/first_medical_encounter_*.csv
#' @import stringr bigrquery data.table
#' @export
first_medical_encounter <- function(dataset,output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
  query <- str_glue("
        WITH ehr AS (
        ...
        SELECT person_id, MIN(date) as first_medical_encounter_entry_date
        FROM ehr
        GROUP BY person_id
        ")
  ...
  bq_table_save(
    ...
    paste0(output_folder,"/aou_phenotyper/first_medical_encounter_*.csv"),
    destination_format = "CSV")
  result <- read_bucket(paste0(output_folder,"/aou_phenotyper/first_medical_encounter_*.csv"))
  if (!is.null(anchor_date_table))
  {
    ...
    result <- result[first_medical_encounter_entry_date >= min_window_date]
    result <- result[first_medical_encounter_entry_date <= max_window_date]
  }
  result <- result[,c("person_id","first_medical_encounter_entry_date")]
  fwrite(result,file="first_medical_encounter.csv")
  system(str_glue("gsutil cp first_medical_encounter.csv {output_folder}/first_medical_encounter.csv"),intern=TRUE)
