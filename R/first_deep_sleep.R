...
#' @export
first_deep_sleep <- function(dataset,output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
  query <- paste("
        SELECT person_id,
  ...
  result <- read_bucket(paste0(output_folder,"/aou_phenotyper/first_deep_sleep_*.csv"))
  if (!is.null(anchor_date_table))
  {
    ...
    result <- result[,c("person_id","first_deep_sleep_datetime","first_deep_sleep_date")]
  }
