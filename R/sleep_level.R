...
#' @return output_folder/sleep_level.csv
...
#' @export
sleep_level <- function(dataset,output_folder,anchor_date_table=NULL,before=NULL,after=NULL)
{
  query <- paste("
        SELECT
        ...
        FROM
            `sleep_level` sleep_level", sep="")
  bq_table_save(
    bq_dataset_query(dataset, query, billing = Sys.getenv("GOOGLE_PROJECT")),
    paste0(output_folder,"/aou_phenotyper/sleep_level_*.csv"),
    destination_format = "CSV")
  result <- read_bucket(paste0(output_folder,"/aou_phenotyper/sleep_level_*.csv"))
  ...
  fwrite(result,file="sleep_level.csv")
  system(str_glue("gsutil cp sleep_level.csv {output_folder}/sleep_level.csv"),intern=TRUE)
